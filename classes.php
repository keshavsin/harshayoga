  ...
  <section id="class_deatils">
      ...
      <div class="container-fluid">
        ...
        <div class="row mrgn_b30">
          <?php
          $modal_box = '';
            $sql = "SELECT * FROM product where type = 'class' AND class_type = 'weekday'";
            $result = $db->query($sql);
            if ($result->num_rows > 0) {
              $indx = 1;
              while($row = $result->fetch_assoc()) {
                echo '<div class="col-xs-12 col-sm-6 col-md-3"><div class="batch_card colr'.$indx.'"><div class="mn_txt">'.$row['duration'].' Months</div><div class="amt_wrap">'.$row['price'].'/-</div><a href="buy_now.php?pid="'.$row["id"].'" class="btn btn-block buy">Buy Now</a><a href="#class_detail'.$indx.'" data-toggle="modal" class="btn btn-block viewdetails">View Details</a></div></div>';
                ...
                $modal_box .= '<div class="modal-footer text-center"><a type="button" class="btn btn-primary btn-lg" href="buy_now.php?pid="'.$row["id"].'">Buy Now</a></div></div></div></div>';
                if($indx == 6){
                  $indx = 1;
                }else{
                $indx++;
        ...
        <div class="row">
          <?php
            $sql1 = "SELECT * FROM product where type = 'class' AND class_type = 'weekend'";
            $result1 = $db->query($sql1);
            if ($result1->num_rows > 0) {
              $indx1 = 5;
              while($row1 = $result1->fetch_assoc()) {
                echo '<div class="col-xs-12 col-sm-6 col-md-3"><div class="batch_card colr'.$indx1.'"><div class="mn_txt">'.$row1['duration'].' Months</div><div class="amt_wrap">'.$row1['price'].'/-</div><a href="#" class="btn btn-block buy">Buy Now</a><a href="" class="btn btn-block viewdetails">View Details</a></div></div>';
                if($indx1 == 6){
                  $indx1 = 1;
                }else{
                $indx1++;
